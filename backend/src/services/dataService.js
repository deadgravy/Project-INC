...
// query for production Count
module.exports.prodCount = async function (startDate, endDate) {
  try {
    const { rows } = await pool.query(
      `SELECT TO_CHAR(DATE_TRUNC('DAY', p.log_time), 'DD/MM/YYYY') as date, COUNT(*)
FROM 
	(SELECT lt.fr_process_steps, lt.log_action, lt.log_time::timestamp FROM log_times lt
	UNION ALL
	SELECT mlt.work_type, mlt.action, mlt.log_time::timestamp FROM manual_log_times mlt) AS p,
...
AND ls.rn = 1
AND p.fr_process_steps = ls.id
AND p.log_time BETWEEN $1 AND $2
...
module.exports.getSingleEquipmentStatus = async function () {
  try {
    const { rows } =
      await pool.query(`SELECT lt.id, pd.name, r.name AS productName, t.fr_process_steps, lt.log_time, pd.mac_property AS singleOrMultiple, lt.log_action, ('2021-08-21 08:00:00' - lt.log_time) AS timeToStarted, rf.queue, ps.process_name, rf.desc_translate, AVG(t.duration)
      FROM ( SELECT  equip_id,
      log_action,
      fr_process_steps,
      start_log_time,
...
module.exports.getAnomolies = async function (start, end, equipmentId) {
  ...
  for (let i = 0; i < equipmentId.length; i++) {
    ...
    if (startRows.length > endRows.length) {
      ...
      anomoliesArr.push({
        x: anomoliesX,
        equipmentName: startRows[i].equipment_name,
      });
    }
  }
